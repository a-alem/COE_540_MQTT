# Code provided by Cisco Packet Tracer - From the python MQTT global project template - with modifications
...
def disable_service():
	global authorized_users, clients, subscriptions, server_enabled, server
		...
		for client in clients:
			if not clients[client].connected():
				...
				for temp_client in temp_clients:
					if temp_client.find("(") >= 0:
						new_client = temp_client[:temp_client.rindex("(") - 1]
