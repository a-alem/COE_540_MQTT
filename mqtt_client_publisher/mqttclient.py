...
def subscribe(topic):
	if topic:
		if topic[0] == "$":
			...
			if on_subscribe_callback is not None:
				on_subscribe_callback("Error", "Not connected to broker.", {})
		else:
			out_packet = {}
...
def unsubscribe(topic):
	if topic:
		if topic[0] == "$":
			...
			if on_unsubscribe_callback is not None:
				on_unsubscribe_callback("Error", "Not connected to broker.", {})
		else:
			out_packet = {}
...
def publish(topic, payload, qos):
	if not topic:
		...
		if on_publish_callback is not None:
			on_publish_callback("Error", "Not connected to broker.", {})
	else:
		out_packet = {}
...
def process_mqtt_packet(in_packet, pdu_info):
	global client, clientID, subscriptions, messages, pingreq_sent, pingresp_received
	
	in_packet = json.loads(in_packet)
	print_event(in_packet)
	
	if in_packet["cmd"] == "ID":
		clientID = getName() + " (" + in_packet["ID"] + ")"
